if(datos.length != 0){
  ...
  /*
    CONOCER CUALES SON LOS GRADOS QUE EXISTEN DENTRO DE UNA INSTITUCIÓN
  */
    var grados = [];
    newDatosDos.forEach(e=>{
      if(grados.includes(e['grado']) != true){
        grados.push(e['grado'])
      }
    })
    
    for(let i = 0; i<grados.length;i++){
      document.getElementById('SG').innerHTML += '<option value="'+grados[i]+'">'+grados[i]+'</option>'
    }
...
function getGrados(){
  ...
  var value = SG.options[SG.selectedIndex].value;
  
  if(value==""){
    document.getElementById('titulosCA').innerHTML ="";
    document.getElementById('resultadoCA').innerHTML = "";
    genTABLA(newDatosDos)
    ...
    document.getElementById('titulosCA').innerHTML ="";
    document.getElementById('resultadoCA').innerHTML = "";
    genTABLA(newDatosTres)
    knowCate(newDatosTres[0])
  }
}
